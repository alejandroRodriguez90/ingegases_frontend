...
const animateStats = () => {
    ...
    const animateNumber = (element, target, prefix) => {
        let start = 0;
        const duration = 2000;
    ...
    const statsObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const prefix = '+'; // Ahora siempre usamos '+' como prefijo
                
                animateNumber(number, target, prefix);
    ...
    statsNumbers.forEach(number => {
        // Inicializar con "+0" en lugar de "0+"
